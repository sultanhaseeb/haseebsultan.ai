...
- **Website articles:** edit `src/data/writing.ts` for featured article metadata and `src/pages/writing/` for article content. The spec-driven development article opens with its image from `public/images/writing/`. Website articles appear alongside the résumé's Medium links on the Writing page and in homepage previews.
